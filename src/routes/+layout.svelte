<script>
	...
	let openingCodeRequired = false;
	let openingCode = '';
	let openingCodeError = '';
	let openingCodeLoading = false;
	...
	const desktopEventHandler = async (event) => {
		...
		if (event.type === 'theme:update' && event.data?.theme) {
			...
			let themeToApply =
				newTheme === 'oled-dark' ? 'dark oled-dark' : newTheme === 'her' ? 'light' : newTheme;
			if (newTheme === 'system') {
				themeToApply = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
			}
			const themePalette = themeToApply.includes('oled-dark') ? 'oled-dark' : themeToApply;
			if (themePalette === 'oled-dark') {
				document.documentElement.style.setProperty('--color-gray-800', '#101010');
				document.documentElement.style.setProperty('--color-gray-850', '#050505');
				document.documentElement.style.setProperty('--color-gray-900', '#000000');
				document.documentElement.style.setProperty('--color-gray-950', '#000000');
			} else if (themePalette === 'dark') {
				document.documentElement.style.setProperty('--color-gray-800', '#333333');
	...
	const submitOpeningCode = async () => {
		const code = openingCode.trim();

		if (!code || openingCodeLoading) {
			return;
		}

		openingCodeLoading = true;
		openingCodeError = '';

		try {
			const status = await verifyOpeningCode(code);

			if (status?.verified) {
				openingCodeRequired = false;
				window.location.reload();
				return;
			}

			openingCodeError = '验证码错误';
		} catch (error) {
			openingCodeError = error?.detail === 'Invalid verification code' ? '验证码错误' : '验证失败';
		} finally {
			openingCodeLoading = false;
		}
	};
	...
	onMount(async () => {
		...
		if (backendConfig?.features?.opening_code?.enabled) {
			try {
				const openingCodeStatus = await getOpeningCodeStatus();

				if (openingCodeStatus?.enabled && !openingCodeStatus?.verified) {
					openingCodeRequired = true;
					openingCodeError = '';
				...
				console.error('Error loading opening code status:', error);
				openingCodeRequired = true;
				openingCodeError = '验证失败';
				document.getElementById('splash-screen')?.remove();
				loaded = true;
				return;
			}
...
{/if}
	...
	{#if loaded && openingCodeRequired}
		<div
			class="fixed inset-0 z-[1000] flex items-center justify-center px-5"
			style="background: var(--owui-bg); color: var(--owui-text-primary);"
		>
			<form
				class="flex w-full max-w-sm flex-col gap-4 rounded-lg border p-6 shadow-xl"
				style="background: var(--owui-surface-raised); border-color: var(--owui-border);"
				on:submit|preventDefault={submitOpeningCode}
			>
				<h1 class="text-xl font-semibold">请输入验证码</h1>

				<input
					class="h-11 rounded-lg border px-3 text-base outline-none transition placeholder:text-gray-500 focus:ring-2 focus:ring-gray-400 dark:placeholder:text-gray-400"
					style="background: var(--owui-surface); border-color: var(--owui-border); color: var(--owui-text-primary);"
					type="password"
					autocomplete="one-time-code"
					aria-label="请输入验证码"
					placeholder="验证码"
					bind:value={openingCode}
					disabled={openingCodeLoading}
					autofocus
				/>

				{#if openingCodeError}
					<div class="text-sm text-red-600 dark:text-red-400">{openingCodeError}</div>
				{/if}

				<button
					class="h-11 rounded-lg bg-gray-900 px-4 text-sm font-medium text-white transition hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-white"
					type="submit"
					disabled={openingCodeLoading || !openingCode.trim()}
				>
					{openingCodeLoading ? '验证中...' : '确认'}
				</button>
			</form>
		</div>
	{:else if loaded}
