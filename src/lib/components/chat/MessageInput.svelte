				...
				<div class="">
					...
					>
						...
						{#if messageQueue.length > 0}
							<div
								class="mb-1 mx-2 py-0.5 px-1.5 rounded-2xl owui-surface-raised border owui-border overflow-x-hidden overflow-y-auto max-h-[25vh]"
							>
						...
						<div
							...
							class="flex-1 flex flex-col relative w-full shadow-lg rounded-3xl border {$temporaryChatEnabled
								...
								: ' owui-border hover:border-gray-200 focus-within:border-gray-100 hover:dark:border-gray-700 focus-within:dark:border-gray-700'}  transition px-1 owui-surface-raised backdrop-blur-sm owui-text-primary"
							dir={$settings?.chatDirection ?? 'auto'}
						>
							{#if atSelectedModel !== undefined}
								<div class="px-3 pt-3 text-left w-full flex flex-col z-10">
									<div class="flex items-center justify-between w-full">
										<div class="pl-[1px] flex items-center gap-2 text-sm owui-text-secondary">
											<img
												alt="model profile"
										...
										<div>
											<button
												class="flex items-center owui-text-secondary hover:text-gray-800 dark:hover:text-white transition"
												on:click={() => {
										...
										{#if file.type === 'image' || (file?.content_type ?? '').startsWith('image/')}
											...
											<div class=" relative group">
												...
												<div class=" absolute -top-1 -right-1">
													<button
														class=" bg-white text-black border border-white rounded-full dark:bg-gray-850 dark:text-gray-100 dark:border-gray-700 {($settings?.highContrastMode ??
														false)
															? ''
										...
										{#if prompt !== '' && !history?.currentId && !$selectedTerminalId && ($config?.features?.enable_notes ?? false) && ($_user?.role === 'admin' || ($_user?.permissions?.features?.notes ?? true))}
											...
											<Tooltip content={$i18n.t('Create note')} className=" flex items-center">
												<button
													...
													class=" text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition rounded-full p-1.5 -mr-1 self-center"
													type="button"
