					...
					{#if searchEnabled}
						<div class="owui-model-search-row flex items-center gap-2.5 px-4.5 pt-3.5 mb-1.5 bg-transparent text-gray-700 dark:text-gray-200">
							...
							<input
								...
								class="w-full text-sm bg-transparent outline-hidden text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
									...
									{#if items.find((item) => item.model?.connection_type === 'local') || items.find((item) => item.model?.connection_type === 'external') || items.find((item) => item.model?.direct) || tags.length > 0}
										<button
											...
											selectedConnectionType === ''
												? ''
												: 'text-gray-300 dark:text-gray-400 hover:text-gray-700 dark:hover:text-white'} transition capitalize"
