<script lang="ts">
	...
	const applyGrayPalette = (_theme: string) => {
		if (_theme === 'oled-dark') {
	...
	const applyTheme = (_theme: string) => {
		let themeToApply =
			_theme === 'oled-dark' ? 'dark oled-dark' : _theme === 'her' ? 'light' : _theme;

		if (_theme === 'system') {
			themeToApply = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
		}

		applyGrayPalette(themeToApply.includes('oled-dark') ? 'oled-dark' : themeToApply);
		...
		<div class="">
			...
			<div class="flex w-full justify-between">
				...
				<div class="flex items-center relative">
					<select
						class="w-fit pr-8 rounded-sm py-2 px-2 text-xs bg-transparent text-right text-gray-800 dark:text-gray-100 {$settings.highContrastMode
							? ''
							: 'outline-hidden'}"
