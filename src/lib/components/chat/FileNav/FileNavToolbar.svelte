...
<div class="flex items-center px-2 pb-1.5 shrink-0 gap-1">
	...
	<Tooltip content={$i18n.t('Back')}>
		<button
			class="shrink-0 p-1 rounded transition {canGoBack
				? 'text-gray-400 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-600 dark:hover:text-gray-400'
	...
	>
		{#each breadcrumbs as crumb, i}
			{#if i > 1}
				<span class="text-gray-300 dark:text-gray-400 text-xs shrink-0 select-none mx-0.5">/</span>
			{/if}
			<button
