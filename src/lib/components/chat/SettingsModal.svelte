<script lang="ts">
	...
	const tabButtonClass = (selected: boolean) => {
		...
		if (selected) {
			return `${base} bg-gray-100 text-gray-900 dark:bg-gray-800 dark:text-gray-100`;
		}

		if ($settings?.highContrastMode ?? false) {
			return `${base} text-gray-800 dark:text-gray-100 hover:bg-gray-200 dark:hover:bg-gray-800`;
		}

		return `${base} text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white hover:bg-gray-100/70 dark:hover:bg-gray-800/70`;
	};
...
<Modal size="2xl" bind:show>
	<div class="owui-text-primary mx-1">
		<div class=" flex justify-between owui-text-secondary px-4 md:px-4.5 pt-4.5 pb-0.5 md:pb-2.5">
		...
		<div class="flex flex-col md:flex-row w-full pt-1 pb-4">
			<div
				...
				class="tabs flex flex-row overflow-x-auto gap-2.5 mx-3 md:pr-4 md:gap-1 md:flex-col flex-1 md:flex-none md:w-50 md:min-h-[min(42rem,calc(100dvh-10rem))] md:max-h-[min(42rem,calc(100dvh-10rem))] owui-text-secondary text-sm text-left mb-1 md:mb-0 -translate-y-1"
			>
				<div
					class="hidden md:flex w-full rounded-full px-2.5 gap-2 owui-surface border owui-border backdrop-blur-2xl my-1 mb-1.5"
					id="settings-search"
				>
					...
					<input
						class={`w-full py-1 text-sm bg-transparent owui-text-primary owui-placeholder outline-hidden
								${($settings?.highContrastMode ?? false) ? 'placeholder:text-gray-800 dark:placeholder:text-gray-100' : ''}`}
						bind:value={search}
				...
				{#if filteredSettings.length > 0}
					...
					{/each}
				{:else}
					<div class="text-center owui-text-muted mt-4">
				...
				{#if $user?.role === 'admin'}
					<a
						href="/admin/settings"
						draggable="false"
						class="{tabButtonClass(false)} md:mt-auto select-none"
						on:click={async (e) => {
							e.preventDefault();
