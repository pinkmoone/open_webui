...
export const getOpeningCodeStatus = async () => {
	let error = null;

	const res = await fetch(`${WEBUI_BASE_URL}/api/opening-code/status`, {
		method: 'GET',
		credentials: 'include',
		headers: {
			'Content-Type': 'application/json'
...
export const verifyOpeningCode = async (code: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_BASE_URL}/api/opening-code/verify`, {
		method: 'POST',
		credentials: 'include',
		headers: {
			'Content-Type': 'application/json'
