...
from open_webui.env import (
    ...
    WEBUI_SECRET_KEY,
    WEBUI_OPENING_CODE,
    WEBUI_OPENING_CODE_EXPIRES_MINUTES,
...
app.state.WEBUI_OPENING_CODE = WEBUI_OPENING_CODE
app.state.WEBUI_OPENING_CODE_EXPIRES_MINUTES = WEBUI_OPENING_CODE_EXPIRES_MINUTES
...
OPENING_CODE_COOKIE_NAME = 'owui-opening-code-verified'


class OpeningCodeForm(BaseModel):
    code: str = ''


def _is_opening_code_enabled() -> bool:
    return bool(app.state.WEBUI_OPENING_CODE)


def _get_opening_code_ttl_seconds() -> int:
    return max(1, app.state.WEBUI_OPENING_CODE_EXPIRES_MINUTES) * 60


def _get_opening_code_signature(payload: str) -> str:
    secret = f'{WEBUI_SECRET_KEY}:{app.state.WEBUI_OPENING_CODE}'.encode('utf-8')
    return hmac.new(secret, payload.encode('utf-8'), hashlib.sha256).hexdigest()


def _create_opening_code_token(expires_at: int) -> str:
    payload = {
        'v': 1,
        'iat': int(time.time()),
        'exp': expires_at,
    ...
    return f'{payload_b64}.{_get_opening_code_signature(payload_b64)}'


def _decode_opening_code_token(token: str) -> Optional[dict]:
    if not token or '.' not in token:
        return None

    try:
        payload_b64, signature = token.split('.', 1)
        expected_signature = _get_opening_code_signature(payload_b64)
...
def _get_opening_code_status(request: Request) -> dict:
    ttl_seconds = _get_opening_code_ttl_seconds()

    if not _is_opening_code_enabled():
        return {
            'enabled': False,
            'verified': True,
            'expires_in': 0,
    ...
    payload = _decode_opening_code_token(request.cookies.get(OPENING_CODE_COOKIE_NAME, ''))
    expires_at = int(payload.get('exp', 0)) if payload else 0
...
@app.get('/api/opening-code/status')
async def get_opening_code_status(request: Request):
    return _get_opening_code_status(request)


@app.post('/api/opening-code/verify')
async def verify_opening_code(form_data: OpeningCodeForm, response: Response):
    ttl_seconds = _get_opening_code_ttl_seconds()
    ...
    submitted_code = (form_data.code or '').strip()
    if not hmac.compare_digest(submitted_code, app.state.WEBUI_OPENING_CODE):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Invalid verification code')

    expires_at = int(time.time()) + ttl_seconds
    response.set_cookie(
        key=OPENING_CODE_COOKIE_NAME,
        value=_create_opening_code_token(expires_at),
        max_age=ttl_seconds,
        httponly=True,
    ...
    return {
        **({'onboarding': True} if onboarding else {}),
        ...
        'features': {
            ...
            'opening_code': {
                'enabled': _is_opening_code_enabled(),
                'ttl_seconds': _get_opening_code_ttl_seconds(),
            },
