...
async function downloadPyPIWheels() {
	const pypiJsonBaseUrl = (
		process.env.PYODIDE_PYPI_JSON_BASE_URL || 'https://pypi.tuna.tsinghua.edu.cn/pypi'
	).replace(/\/$/, '');
	const pypiFileBaseUrl = (
		process.env.PYODIDE_PYPI_FILE_BASE_URL || 'https://pypi.tuna.tsinghua.edu.cn'
	).replace(/\/$/, '');
	...
	for (const pkg of pypiPackages) {
		...
		const res = await fetch(`${pypiJsonBaseUrl}/${pkg}/json`);
		if (!res.ok) {
		...
		try {
			...
			const wheelUrl = wheel.url.replace(/^https:\/\/files\.pythonhosted\.org/, pypiFileBaseUrl);
